...
def read_excel_file(filename):
    # Получаем путь к файлу Excel, находящемуся в той же папке, что и скрипт
    script_dir = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(script_dir, filename)

    # Читаем файл Excel, указывая явно движок 'openpyxl' и тип данных для первой колонки
...
def recognize_digits(left, top, width, height, product_name, product_price, queue):
    try:
        excel_data = read_excel_file("products2.xlsx")
        index = 0
        keyboard.wait("enter")
        ...
        while True:
            ...
            row = excel_data[index % len(excel_data)]
            first_element, second_element = row
            pyautogui.moveTo(x=1700, y=450, duration=0.1)
            pyautogui.click(x=1700, y=450, duration=0.01)
            paste(first_element)
            pyautogui.moveTo(x=1790, y=450, duration=0.2)
            pyautogui.click(x=1790, y=450, duration=0.01)
            sleep(0.8)  
            ...
            for i, text in enumerate(merged_data["text"]):
                if text.isdigit():  # Проверяем, является ли текст числом
                    number = int(text)
                    if number <= second_element and number > 1000:
                        print(">price", number)
                        pyautogui.moveTo(
                        ...
                        pyautogui.moveTo(x=1260, y=760, duration=0.17)
                        pyautogui.click(x=1260, y=760, duration=0.05)
